#!/usr/bin/env python3
"""Build Rich Tillman's ATS-safe resume as a single submission for the SQUAD recruiter to
use across all 3 of their client roles: E3 Group (Applied Engineer), GC AI (Member of
Technical Staff, Applied AI), and GC AI (Member of Technical Staff, Product Engineering).
   python3 you/build-ats-resume-squad.py
Outputs /tmp/pdfout/Rich_Tillman_Resume_SQUAD.docx and you/resume-squad.html.
...
Why this positioning: rather than diluting toward none of the three roles, the SUMMARY
covers the load-bearing claim each JD cares about most: LLMs consumed as a production API
with real latency/cost/UX tradeoffs (E3, GC AI Applied AI), the current AI Model Evaluation
& Red-Teaming consulting work (near-literal match for GC AI Applied AI's per-release
evaluation requirement), and full-stack ownership from API to UI including real GCP/
Kubernetes architecture (GC AI Product Engineering). All three roles came from the same
recruiter (SQUAD Talent Acquisition), so one resume, not three, is what gets submitted.
"""
import zipfile, html, os
def esc(t): return html.escape(t, quote=False)

NAME="RICH TILLMAN"; TITLE="Full-Stack Engineer, AI-Native Product Development"
CONTACT="Elizabethton, TN | 843-834-0041 | richtillman@pm.me | linkedin.com/in/effinrich | github.com/effinrich | richtillman.xyz"
SUMMARY=("Full-stack engineer, 15 years, who builds production software with LLMs as a consumed API and ships "
 "full-stack features end to end, not just frontend. I'm currently doing paid contract work evaluating and "
 "red-teaming frontier AI model outputs against scoring rubrics across four LLMs (Mercor, micro1, Handshake), the "
 "same evaluation discipline any team running multiple production models needs on every release. I've built and "
 "shipped LLM-orchestration products with real latency, cost, and UX tradeoffs: Santa Chat AI and Textation combine "
 "an LLM chat backend with ElevenLabs text-to-speech, Deepgram speech-to-text, and image/video generation, with "
 "Stripe-paying users and an ElevenLabs 300K-token grant along the way. Full-stack ownership from API to UI is how "
 "I've always worked: NestJS backends at Freebird and Pineapple, founding architecture on GCP and Kubernetes with "
 "gRPC middleware for PHC Global's enterprise platform (used by Facebook and Amex), and PostgreSQL via Supabase with "
 "RLS on recent projects. I author production MCP servers (ForgeKit, 6,200+ npm installs) rather than just consuming "
 "AI tools, and Claude Code and Cursor, including Cursor's background agent, are in daily production use for me. "
 "Promoted twice to Engineering Director at Redesign Health, with real startup-scale ownership across five 0-to-1 "
 "companies and a habit of mentoring engineers, not just shipping solo.")
JOBS=[
...
PROJECTS=[
 ...
 ("TokenCast (tokencast.vercel.app)",
  "Full-stack app: paste design tokens and get a live preview plus exportable theme code, with server-rendered Save & Share links. React Router (framework mode: SSR, loaders, actions), React 19, TypeScript, Supabase (RLS + SECURITY DEFINER RPCs, IP rate-limiting), Vitest + Playwright; Lighthouse 100 accessibility, 99 performance."),
 ("MCP Atlas (mcp-atlas-orcin.vercel.app)",
  "Curated, link-verified directory of Model Context Protocol servers (18 official + community, searchable and filterable). Astro (Zod-validated content collections + islands architecture), TypeScript, Vitest, and Playwright; Lighthouse 100 accessibility, 97 performance."),
 ("Tidy App (tidyapp.me)",
...
SKILLS=[
 ...
 ("Agentic AI Development","Model Context Protocol (MCP) - author production MCP servers, not just a consumer; Claude Code and Cursor (including Cursor's background/autonomous agent) in daily production use; ChatGPT Pro; AI model evaluation & red-teaming across frontier LLMs (adversarial testing of AI outputs)"),
 ("LLM Integration & Orchestration","LLM chat backends, ElevenLabs text-to-speech & Conversational AI, Deepgram speech-to-text, image/video generation (Stable Diffusion, SadTalker); production latency/cost/UX tradeoffs of serving models"),
 ...
 ("Cloud & Infrastructure","GCP, Kubernetes, gRPC, Vite, Nx, Turborepo, pnpm workspaces, Module Federation, Oxlint, GitHub Actions CI/CD, EAS Build"),
...
open(os.path.join(here,"resume-squad.html"),'w').write("<!DOCTYPE html><html><head><meta charset='utf-8'>"+CSS+"</head><body>"+''.join(P)+"</body></html>")
...
out='/tmp/pdfout/Rich_Tillman_Resume_SQUAD.docx'
