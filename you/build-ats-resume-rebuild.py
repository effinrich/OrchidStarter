#!/usr/bin/env python3
"""Build Rich Tillman's ATS-safe resume, tailored for Re:Build Manufacturing's Reflow
product (Senior Frontend Engineer, $165K-$200K) - a manufacturing-operations platform.
   python3 you/build-ats-resume-rebuild.py
Outputs /tmp/pdfout/Rich_Tillman_Resume_Rebuild.docx and you/resume-rebuild.html.
...
Gate check: role requires US-person status for export control (ITAR-adjacent) - Rich is a
US citizen, confirmed before this was built.
Why tailored: JD leans on Vite, Tailwind, headless component libraries (Radix/Ark UI),
OpenAPI-driven codegen, Playwright/Cypress, and JWT/OIDC auth - all genuinely on Rich's
resume now (added to the base after confirming with him), plus real experience building
data-dense operational software (PHC Global's COVID risk-mapping/logistics platform,
Freebird's B2B/B2C dashboards) that maps directly onto Reflow's manufacturing-ops domain.
"""
import zipfile, html, os
def esc(t): return html.escape(t, quote=False)

NAME="RICH TILLMAN"; TITLE="Senior Frontend Engineer"
CONTACT="Elizabethton, TN | 843-834-0041 | richtillman@pm.me | linkedin.com/in/effinrich | github.com/effinrich | richtillman.xyz"
SUMMARY=("Senior Frontend Engineer, 15 years, with a track record of building data-dense operational software: a COVID "
 "risk-mapping and logistics platform used by enterprise clients (including Facebook and Amex) to schedule ground teams "
 "and plan routes at PHC Global, and B2B/B2C operational dashboards at Freebird. Deep design-systems and headless-"
 "component-library experience (Radix UI, Ark UI, Storybook, Tailwind CSS) building UI that has to stay legible under "
 "real operational complexity, not just look good in isolation. Hands-on with the exact tooling this role calls for: "
 "Vite (drove the migration and became my team's default bundler), OpenAPI/Swagger-generated React Query hooks and "
 "server functions, Playwright and Cypress for e2e, and JWT auth (collaborated with backend across three companies) "
 "plus OIDC migration spec work. Nx monorepo architecture and shared-UI-library patterns built repeatedly across five "
...
PROJECTS=[
 ...
 ("ForgeKit - independent open-source developer-tooling venture (2024 - 2026)",
  "Open-source CLI + MCP server suite bridging Figma to React to Storybook for design-system teams. 6,200+ npm installs across three separate first-time package releases, with active production adoption. forgekit.cloud, github.com/effinrich, npmjs.com."),
...
SKILLS=[
 ...
 ("Design Systems & Headless UI","Storybook 10+, Chromatic, Radix UI, Ark UI, Chakra UI, shadcn/ui, Tamagui, Tailwind CSS, Styled Components / CSS-in-JS, Figma Code Connect"),
 ...
 ("APIs, Auth & Data","REST, GraphQL, tRPC, OpenAPI/Swagger (client & hook codegen), JWT, OIDC (migration spec design), Redux Toolkit, Zustand, TanStack Query, Supabase, PostgreSQL, Context API"),
 ("Testing & Accessibility","Playwright, Cypress, Vitest, Jest, React Testing Library, WCAG 2.1 AA"),
 ("Agentic AI Development","Model Context Protocol (MCP) - author production MCP servers; Claude Code and Cursor (including Cursor's background/autonomous agent) in daily production use; ChatGPT Pro; AI model evaluation & red-teaming across frontier LLMs"),
...
open(os.path.join(here,"resume-rebuild.html"),'w').write("<!DOCTYPE html><html><head><meta charset='utf-8'>"+CSS+"</head><body>"+''.join(P)+"</body></html>")
...
out='/tmp/pdfout/Rich_Tillman_Resume_Rebuild.docx'
