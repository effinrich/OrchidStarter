#!/usr/bin/env python3
"""Build Rich Tillman's ATS-safe resume, tailored for Anthropic's Design Engineer, Web
role (Creative Studio / Brand Web team, anthropic.com + claude.com). Edit data below, then:
   python3 you/build-ats-resume-anthropic-design.py
Outputs /tmp/pdfout/Rich_Tillman_Resume_Anthropic_DesignEng.docx and you/resume-anthropic-design.html.
...
Honesty note: Rich has NO dedicated headless-CMS-platform experience (Contentful/Sanity/
Strapi/etc.) and NO localization/i18n production experience - both are explicit "good fit"
requirements for this role, not just bonus. Closest honest precedents: a "real-time CMS on
Firebase Realtime Database" at FaceCake, and Zod-validated content collections (Astro) on
MCP Atlas - both are named plainly, neither is dressed up as more than it is. Everything
else (design systems, AI-powered dev/creative tooling with real adoption via ForgeKit/MCP,
architecture ownership, craft, Core Web Vitals via Lighthouse scores) is a strong, honest match.
"""
...
SUMMARY=("Principal Frontend Engineer with 15 years building production design systems and the AI-powered creative/developer "
 ...
 "localization production experience - closest precedents are a real-time CMS built on Firebase and Zod-validated content "
 "modeling on a recent project; genuinely excited to grow into both. Seeking Principal / Senior Design Engineer and "
 "design-systems / developer-tooling roles where craft and AI-native workflows meet.")
JOBS=[
...
SKILLS=[
 ...
 ("DevOps & Monorepo","Nx, Turborepo, pnpm workspaces, Module Federation, Vite, GitHub Actions, EAS Build, GCP, Kubernetes, gRPC"),
...
open(os.path.join(here,"resume-anthropic-design.html"),'w').write("<!DOCTYPE html><html><head><meta charset='utf-8'>"+CSS+"</head><body>"+''.join(P)+"</body></html>")
...
out='/tmp/pdfout/Rich_Tillman_Resume_Anthropic_DesignEng.docx'
