#!/usr/bin/env python3
"""Build Rich Tillman's ATS-safe resume, tailored for Axci Capital's Venture Fellow,
Technology & Research role - a builder role inside an operator-led VC firm: multi-agent
systems for startup discovery, market intelligence, and diligence workflows.
   python3 you/build-ats-resume-axci.py
Outputs /tmp/pdfout/Rich_Tillman_Resume_Axci.docx and you/resume-axci.html.
...
Why tailored: the posting explicitly says "we care more about what you've built than the
title on your resume" and wants someone who turns ambiguous problems into working systems.
ForgeKit (production MCP-server authorship) is real agentic infrastructure, the AI Model
Evaluation & Red-Teaming consulting work is hands-on multi-model experimentation, and
Santa Chat AI/Textation are shipped products wiring multiple AI services into one system -
all close matches. Honest gaps named plainly: no Python on the stack, no formal multi-agent
framework experience (LangGraph, CrewAI, AutoGen), no venture/finance background.
"""
import zipfile, html, os
def esc(t): return html.escape(t, quote=False)

NAME="RICH TILLMAN"; TITLE="Venture Fellow, Technology & Research"
CONTACT="Elizabethton, TN | 843-834-0041 | richtillman@pm.me | linkedin.com/in/effinrich | github.com/effinrich | richtillman.xyz"
SUMMARY=("Builder first, 15 years, with a habit of turning fragmented, ambiguous problems into working systems, not "
 "research documents. I author production MCP servers (ForgeKit, 6,200+ npm installs) that give AI coding agents the "
 "context to work correctly: real agentic infrastructure, not a one-off experiment. I'm currently doing paid contract "
 "work evaluating and red-teaming frontier AI model outputs across four LLMs (Mercor, micro1, Handshake), hands-on "
 "comparison of what different models actually do well, not theoretical familiarity. I've shipped products that wire "
 "multiple AI services into one coordinated system: Santa Chat AI and Textation combine an LLM chat backend with "
 "ElevenLabs text-to-speech, Deepgram speech-to-text, and image and video generation, with real paying users and an "
 "ElevenLabs 300K-token grant along the way. Founding or near-founding engineer at three of my last five companies, "
 "with the instinct this role wants: given a problem instead of a spec, I build a prototype and iterate. Honest gaps: "
 "no Python on my day-to-day stack (TypeScript and JavaScript are home base), no formal multi-agent framework "
 "experience (LangGraph, CrewAI, AutoGen) specifically, and no venture or finance background.")
JOBS=[
...
PROJECTS=[
 ...
 ("MCP Atlas (mcp-atlas-orcin.vercel.app)",
  "Curated, link-verified directory of Model Context Protocol servers (18 official + community, searchable and filterable), built and maintained as public open source. Astro (Zod-validated content collections + islands architecture), TypeScript, Vitest, and Playwright; Lighthouse 100 accessibility, 97 performance."),
 ...
 ("TokenCast (tokencast.vercel.app)",
  "Paste design tokens (Figma variables, CSS custom properties, or a Tailwind config) and get a live preview plus exportable theme code for Tailwind, Chakra, and shadcn/ui, with server-rendered Save & Share links. React Router (framework mode: SSR, loaders, actions), React 19, TypeScript, Supabase (RLS + SECURITY DEFINER RPCs, IP rate-limiting), Vitest + Playwright; Lighthouse 100 accessibility, 99 performance."),
 ("Tidy App (tidyapp.me)",
...
SKILLS=[
 ...
 ("LLM Integration & Orchestration","LLM chat backends, ElevenLabs text-to-speech & Conversational AI, Deepgram speech-to-text, image/video generation (Stable Diffusion, SadTalker); connecting multiple AI services and APIs into one working system"),
 ("Languages","JavaScript (15 yrs), TypeScript (strict), HTML5, CSS3, SQL, PHP (legacy) - no Python day to day"),
 ("APIs & Data","REST, GraphQL, tRPC, OpenAPI/Swagger (client & hook codegen), JWT, Redux Toolkit, Zustand, TanStack Query, Supabase, PostgreSQL, MongoDB, Context API"),
...
open(os.path.join(here,"resume-axci.html"),'w').write("<!DOCTYPE html><html><head><meta charset='utf-8'>"+CSS+"</head><body>"+''.join(P)+"</body></html>")
...
out='/tmp/pdfout/Rich_Tillman_Resume_Axci.docx'
