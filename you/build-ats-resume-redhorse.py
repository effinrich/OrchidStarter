#!/usr/bin/env python3
"""Build Rich Tillman's ATS-safe resume, tailored for Redhorse Corporation's "Everything
Engineer" (Full-Stack, Frontend Focus) role - modernizing a federal foreign-military-sales
system with an Agentic AI SDLC. Edit data below, then:
   python3 you/build-ats-resume-redhorse.py
Outputs /tmp/pdfout/Rich_Tillman_Resume_Redhorse.docx and you/resume-redhorse.html.
...
Gate check: role requires US citizenship + SECRET clearance eligibility - Rich confirmed
US citizen, eligible, no known disqualifiers, before this was built.
Honesty note: real gaps named plainly, not glossed - no AWS/AWS CDK (cloud background is
GCP/Kubernetes), no Prisma specifically (PostgreSQL yes, via Supabase), no PDF/document-
generation/rendering experience, no government/regulated-environment experience (JD lists
this as nice-to-have, not required). Rarest, strongest match: genuine hands-on agent-native
development - ForgeKit IS harness/spec tooling for AI coding agents, which is close to
exactly what "you have opinions about specs, context, and harness design" is asking for.
"""
...
SKILLS=[
 ...
 ("DevOps & Cloud","Nx, Turborepo, pnpm workspaces, Module Federation, GitHub Actions CI/CD, EAS Build, GCP, Kubernetes, gRPC (no production AWS - see Summary)"),
...
open(os.path.join(here,"resume-redhorse.html"),'w').write("<!DOCTYPE html><html><head><meta charset='utf-8'>"+CSS+"</head><body>"+''.join(P)+"</body></html>")
...
out='/tmp/pdfout/Rich_Tillman_Resume_Redhorse.docx'
