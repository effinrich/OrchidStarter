#!/usr/bin/env python3
"""Build Rich Tillman's ATS-safe resume, tailored for E3 Group's Applied Engineer role -
the customer-facing "Applied" track (vs. Systems/Research) at a freight/supply-chain
applied-AI lab.
   python3 you/build-ats-resume-e3group.py
Outputs /tmp/pdfout/Rich_Tillman_Resume_E3Group.docx and you/resume-e3group.html.
...
Why tailored: JD wants React/REST production customer-facing features, LLMs consumed as an
API with real latency/cost/UX judgment (not just prompting), heavy fluent daily coding-agent
use (assessed directly in interview), and shipped-to-a-real-customer proof. Santa Chat AI /
Textation are real production LLM-orchestration products with paying users; the current AI
Model Evaluation & Red-Teaming consulting work is close to a literal match for "heavy,
fluent daily use of coding agents." Honest note: data-architecture ownership has been in
service of frontend systems (NestJS APIs, Supabase/PostgreSQL, gRPC middleware), not a
dedicated backend-architect role - named plainly rather than glossed over.
"""
import zipfile, html, os
def esc(t): return html.escape(t, quote=False)

NAME="RICH TILLMAN"; TITLE="Applied Engineer"
CONTACT="Elizabethton, TN | 843-834-0041 | richtillman@pm.me | linkedin.com/in/effinrich | github.com/effinrich | richtillman.xyz"
SUMMARY=("Applied Engineer, 15 years shipping customer-facing React/REST products for real, paying users - the applied, "
 "not research, seat this role is. I've built and shipped LLM-powered products as a consumed API, not just prompted one: "
 "Santa Chat AI and Textation (LLM chat backend wired to ElevenLabs text-to-speech, Deepgram speech-to-text, and image/"
 "video generation) required real decisions about latency, cost, and UX tradeoffs serving models to production traffic - "
 "Santa Chat AI was selected by ElevenLabs for a 300K-token grant, and both run on Stripe-paying subscriptions. "
 "Genuinely AI-native day to day: Claude Code and Cursor (including Cursor's background agent) in daily production use, "
 "author of production MCP servers, and currently doing paid contract work evaluating and red-teaming frontier LLM "
 "outputs across four models - fluent coding-agent use is my day job, not something to demonstrate cold. Direct "
 "customer-facing experience too: design-engineering liaison across client, sales, and marketing at Freebird, and "
 "founding architecture (Nx monorepo, gRPC middleware, GCP/Kubernetes infrastructure decisions) for PHC Global's "
 "enterprise platform used by Facebook and Amex. Honest note: data-architecture work has been in service of frontend "
 "systems (NestJS APIs, Supabase/PostgreSQL with RLS), not a dedicated backend-architect role - but I've made real "
 "scale-driving infrastructure calls, not just feature-level ones.")
JOBS=[
...
open(os.path.join(here,"resume-e3group.html"),'w').write("<!DOCTYPE html><html><head><meta charset='utf-8'>"+CSS+"</head><body>"+''.join(P)+"</body></html>")
...
out='/tmp/pdfout/Rich_Tillman_Resume_E3Group.docx'
