...
Applied AI role - full-stack engineering with a strong AI execution layer (explicitly NOT
an ML/research seat), heavy on LLM orchestration, per-release model evaluation, and
rapid-prototyping frameworks as new models ship weekly.
   python3 you/build-ats-resume-gcai-appliedai.py
Outputs /tmp/pdfout/Rich_Tillman_Resume_GCAI_AppliedAI.docx and you/resume-gcai-appliedai.html.
...
Why tailored: the current AI Model Evaluation & Red-Teaming consulting work (Mercor, micro1,
Handshake) is close to a literal match for "run evaluation and testing work on every model
release." Santa Chat AI / Textation are real, shipped LLM-orchestration products (LLM chat +
ElevenLabs TTS + Deepgram STT + image/video gen). ForgeKit is production MCP-server
authorship, not just AI-tool consumption. No RAG-systems or "founding engineer with a
notable exit" experience (both explicitly bonus, not required) - not claimed.
"""
import zipfile, html, os
def esc(t): return html.escape(t, quote=False)

NAME="RICH TILLMAN"; TITLE="Member of Technical Staff, Applied AI"
CONTACT="Elizabethton, TN | 843-834-0041 | richtillman@pm.me | linkedin.com/in/effinrich | github.com/effinrich | richtillman.xyz"
SUMMARY=("Full-stack engineer with a genuine AI execution layer, not a machine-learning or research background - the "
 "exact seat this role is. I'm currently doing paid contract work evaluating and red-teaming frontier AI model outputs "
 "against scoring rubrics across four LLMs (sequential engagements with Mercor, micro1, and Handshake) - the same "
 "evaluation discipline this role needs applied to GC AI's 4-5 production models on every release. I've built and "
 "shipped LLM-orchestration products end to end: Santa Chat AI and Textation combine an LLM chat backend with ElevenLabs "
 "text-to-speech, Deepgram speech-to-text, and image/video generation into one live product (Stripe-paying "
 "subscriptions; ElevenLabs selected Santa Chat AI for a 300K-token grant). I author production MCP servers, not just "
 "consume AI tools - ForgeKit (6,200+ npm installs across three first-time package releases) is itself harness/spec "
 "tooling that gives AI coding agents the context to work correctly - and Claude Code/Cursor (including Cursor's "
 "background agent) are in daily production use for me. Fifteen years of deep, hands-on TypeScript/React with real "
 "startup-scale ownership across five 0-to-1 companies, and mentorship as a habit: I built the onboarding methodology "
 "that took backend engineers to production-level React in one month at Redesign Health.")
JOBS=[
...
open(os.path.join(here,"resume-gcai-appliedai.html"),'w').write("<!DOCTYPE html><html><head><meta charset='utf-8'>"+CSS+"</head><body>"+''.join(P)+"</body></html>")
...
out='/tmp/pdfout/Rich_Tillman_Resume_GCAI_AppliedAI.docx'
