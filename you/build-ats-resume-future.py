#!/usr/bin/env python3
"""Build Rich Tillman's ATS-safe resume, tailored for Future's Staff Frontend Engineer role
(member-facing product, personalized health/coaching). Edit data below, then:
   python3 you/build-ats-resume-future.py
Outputs /tmp/pdfout/Rich_Tillman_Resume_Future.docx and you/resume-future.html.
...
Honesty note: Rich's production Next.js experience (Santa Chat AI, Textation) is Next.js 12 /
Pages Router - predates App Router (shipped in Next.js 13). The JD explicitly asks for App
Router production experience. TokenCast uses React Router's framework mode (SSR/loaders/
actions), conceptually similar but not literally App Router. Named honestly in the summary,
not glossed over.
Personal note: Rich asked to weave in genuine gym/nutrition dedication, since the JD explicitly
lists "Domain Passion" (health/fitness/coaching) as a "Who You Are" criterion - this is a real
scored signal for this specific role, not filler. PLACEHOLDER LANGUAGE BELOW - Rich needs to
supply real specifics (how long training, what kind, nutrition approach) rather than have this
invented. Search "PLACEHOLDER" below before sending.
"""
...
open(os.path.join(here,"resume-future.html"),'w').write("<!DOCTYPE html><html><head><meta charset='utf-8'>"+CSS+"</head><body>"+''.join(P)+"</body></html>")
...
out='/tmp/pdfout/Rich_Tillman_Resume_Future.docx'
